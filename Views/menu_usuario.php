...
<div class="modal" id="infoMascotaModal">
    <div class="modal-content">
        ...
        <img id="mascotaImagen" src="" alt="Imagen de la mascota">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    botonesAdoptar.forEach(boton => {
        boton.addEventListener("click", function () {
            ...
            fetch("/Animazing/Acciones/adoptar.php", { // 🔹 Ruta absoluta
                method: "POST",
                headers: {
...
<!-- Script del leer más -->
<script>
// Delegar evento de clic a todos los botones .leer
document.querySelectorAll('.leer').forEach(boton => {
    boton.addEventListener('click', function() {
        const idMascota = this.getAttribute('data-id');

        fetch("../Acciones/mascota_info.php", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: "id_mascota=" + encodeURIComponent(idMascota)
        })
        ...
        .then(data => {
            ...
            // Si tienes imagen en la DB
            if (data.imagen) {
                document.getElementById("mascotaImagen").src = data.imagen;
            }

            document.getElementById("infoMascotaModal").style.display = "flex";
        })
        .catch(error => {
